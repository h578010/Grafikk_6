import ShaderCustomiser from './ShaderCustomiser';
import { ShaderMaterial, Color, ShaderLib, UniformsLib, UniformsUtils, Texture } from 'three';
...
export default class TextureSplattingMaterial extends ShaderMaterial {
    ...
    constructor(color: number, shininess:number, textures:Texture[], splatMaps:Texture[]) {
        ...
        if (textures !== null && splatMaps !== null) {
            ...
            uniforms.textureUvTransforms = {
                type: "Matrix3fv",
                value: (textures! as []).map((texture:any) => {
        ...
        const length = (textures !== null) ? (textures! as []).length : 0;
...
`;
        ...
        const splatmap_blending_code = (i:number, length:number):string => {
            if (i < length) {
        ...
        const splatmap_code = `
#ifdef USE_SPLATMAP
    float splatSum = ${Array(length - 1).fill(undefined).map((_, i) => `texture2D(splatMaps[${i}], vUv).r`).join(' + ')};
